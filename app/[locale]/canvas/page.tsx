...
import {
  parseCanvasEditCommand,
  resolveShapeIdsByHint,
...
export default function CanvasPage() {
  ...
  const presetTemplates = useMemo(
    () =>
      [
        {
          icon: "Sun",
          title: tCanvas("presetTemplates.sunrise.title"),
          prompt: tCanvas("presetTemplates.sunrise.prompt"),
        },
        {
          icon: "Home",
          title: tCanvas("presetTemplates.house.title"),
          prompt: tCanvas("presetTemplates.house.prompt"),
        },
        {
          icon: "Flower2",
          title: tCanvas("presetTemplates.flower.title"),
          prompt: tCanvas("presetTemplates.flower.prompt"),
        },
        {
          icon: "Star",
          title: tCanvas("presetTemplates.stars.title"),
          prompt: tCanvas("presetTemplates.stars.prompt"),
        },
        {
          icon: "Cat",
          title: tCanvas("presetTemplates.animal.title"),
          prompt: tCanvas("presetTemplates.animal.prompt"),
        },
        {
          icon: "Palette",
          title: tCanvas("presetTemplates.abstract.title"),
          prompt: tCanvas("presetTemplates.abstract.prompt"),
        },
      ] as const,
    [tCanvas],
  );
  ...
  // 绘制预设动画图形
  const drawPresetShape = useCallback(
    (
  ...
  const animatePresetShapes = useCallback(
    function runPresetShapesAnimation() {
      ...
      shapeAnimationRef.current = requestAnimationFrame(
        runPresetShapesAnimation,
      );
    },
  ...
  const toTopLeft = useCallback((shape: Shape) => {
    ...
    switch (shape.type) {
      case "circle": {
        ...
        const d = r * 2;
        if (anchor === "center") return { x: shape.x - r, y: shape.y - r };
        return { x: shape.x - d, y: shape.y - d }; // bottom-right
      }
      case "ellipse": {
        const rx = shape.rx || 50;
        const ry = shape.ry || 30;
        if (anchor === "center") return { x: shape.x - rx, y: shape.y - ry };
        return { x: shape.x - rx * 2, y: shape.y - ry * 2 }; // bottom-right
      }
      case "polygon": {
  ...
  const getShapeBounds = useCallback(
    (shape: Shape) => {
  ...
  const resolvePositionCenter = useCallback(
    (
      position:
        | "top-left"
        | "top-right"
  ...
  const drawSingleShape = useCallback(
    (ctx: CanvasRenderingContext2D, shape: Shape) => {
      ...
      if (shape.sketch) {
        const { points: outline, closed } = flattenShapeOutline(
          shape,
          toTopLeft,
        );
        if (outline.length >= 2) {
          const { points: jittered, widthScale } = applySketchJitter(
            outline,
            shape.sketch,
          );
          ...
          if (fill && closed) {
            ...
            for (let i = 1; i < jittered.length; i++)
              ctx.lineTo(jittered[i].x, jittered[i].y);
            ctx.closePath();
            ctx.fill();
          }
          // 清除阴影后用抖动折线描边（带线宽脉动）
  ...
  // 计算当前进度下画笔应该在的位置
  const getBrushPositionAtProgress = useCallback(
    (shape: Shape, progress: number): { x: number; y: number } => {
      switch (shape.type) {
        case "rectangle": {
          const w = shape.width || 100;
          const h = shape.height || 100;
  ...
  const drawProgressiveShape = useCallback(
    (ctx: CanvasRenderingContext2D, shape: Shape, progress: number) => {
      const opacity = shape.opacity ?? 1;
  ...
  const redrawFromState = useCallback(
    (state: CanvasState) => {
      const canvas = canvasRef.current;
      if (!canvas) return;
      const ctx = canvas.getContext("2d");
      ...
      if (state.vignette?.strength) {
        ...
        grad.addColorStop(
          1,
          `rgba(0,0,0,${Math.max(0, Math.min(1, state.vignette.strength)) * 0.6})`,
        );
        ctx.save();
        ctx.fillStyle = grad;
        ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
  ...
  // 加载作品进行编辑
  const loadArtworkForEdit = useCallback(
  ...
  const drawShapes = useCallback(
    async (instructions: DrawInstruction) => {
      ...
      if (instructions.vignette && instructions.vignette.strength > 0) {
        const strength = Math.max(
          0,
          Math.min(1, instructions.vignette.strength),
        );
    ...
    [
      drawProgressiveShape,
      drawSingleShape,
      getBrushPositionAtProgress,
      getShapeBounds,
      getOffscreenCanvas,
    ],
  );

  const drawAppendBatch = useCallback(
    async (committedState: CanvasState, incomingShapes: Shape[]) => {
      const canvas = canvasRef.current;
      if (!canvas) return;
      const ctx = canvas.getContext("2d");
      if (!ctx) return;
      ...
      offCtx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
      for (const shape of [...committedState.shapes].sort(
        (a, b) => (a.z ?? 0) - (b.z ?? 0),
      )) {
        drawSingleShape(offCtx, shape);
      }
      ...
      for (const shape of [...incomingShapes].sort(
        (a, b) => (a.z ?? 0) - (b.z ?? 0),
      )) {
        const bounds = getShapeBounds(shape);
      ...
      if (committedState.vignette?.strength) {
        ...
        grad.addColorStop(
          1,
          `rgba(0,0,0,${Math.max(0, Math.min(1, committedState.vignette?.strength ?? 0)) * 0.6})`,
        );
        ctx.save();
        ctx.fillStyle = grad;
        ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
  ...
  const handleSaveClick = useCallback(() => {
    const defaultTitle =
      sessionDescription.substring(0, 30) || tCanvas("saveUntitled");
  ...
  const exportPNG = useCallback(() => {
    ...
    try {
      ...
      const title =
        sessionDescription.substring(0, 30) || tCanvas("fallbackDownloadName");
  ...
  const applyLocalEditCommand = useCallback(
    async (prompt: string) => {
      const command = parseCanvasEditCommand(prompt);
      if (command.kind === "none") return false;

      const targetIds = resolveShapeIdsByHint(
        canvasState.shapes,
        command.targetHint,
        activeShapeIds,
      );
      ...
      if (command.kind === "move") {
        const targetShapes = canvasState.shapes.filter(
          (shape) => shape.id && targetIds.includes(shape.id),
        );
  ...
  const handleStartDrawing = useCallback(async () => {
    ...
    try {
      ...
      const similarPromptResult = await findSimilarPrompt(
        sessionDescription,
        0.9,
      );
      ...
      if (similarPromptResult.matched && similarPromptResult.history) {
        ...
        if (!response.ok) {
          const errorPayload = (await parseJsonSafely(response).catch(
            () => null,
      ...
      addToast(
        "error",
        error instanceof Error ? error.message : tCanvas("drawFailed"),
      );
    } finally {
      setIsDrawing(false);
      setIsAppending(false);
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-3 pb-3 pt-3">
        <div className="mx-auto flex h-full w-full max-w-[1600px] flex-col gap-3 overflow-hidden">
          ...
          <div className="flex min-h-0 flex-1 items-center gap-3 overflow-hidden">
            ...
            <div
              ref={canvasAreaRef}
              className="relative aspect-[4/3] h-auto max-h-full w-full max-w-[480px] flex-[0_1_480px] overflow-hidden rounded-3xl border border-sakura/15 bg-gradient-to-br from-white to-sakura-light/10 shadow-xl shadow-sakura/8"
            >
              {/* 背景装饰网格 */}
              <div
              ...
              {isThinking && (
                <div className="absolute inset-0 bg-white/60 backdrop-blur-sm flex flex-col items-center justify-center z-10">
                  ...
                  <div className="relative w-32 h-32 mb-6">
                    ...
                    <div
                      className="absolute inset-4 border-4 border-lavender/30 rounded-full animate-ping"
                      style={{ animationDelay: "0.3s" }}
                    />
                    <div
                      className="absolute inset-8 border-4 border-macaron-blue/40 rounded-full animate-ping"
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        <h3 className="text-lg font-bold text-text-primary">
                          {presetTemplates[currentPresetIndex].title}
                        </h3>
                        <p className="text-xs text-text-secondary">
                          {tCanvas("inspirationExample")}
                        </p>
                      </div>
                    </div>
              ...
              {/* 静息状态引导覆盖层 - 画布空白时显示 */}
              <IdleGuide
                visible={!isThinking && canvasState.shapes.length === 0}
              />
                ...
                <div className="p-4 pt-2">
                  ...
                  <div className="mt-2 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2 text-xs text-text-secondary">
                      <span className="rounded-full bg-sakura-light px-2 py-1 text-sakura">
                        {tCanvas("singleScreenLayout")}
                      </span>
                      <span>
                        {canvasState.shapes.length > 0
                          ? tTeaching("appendHint")
                          : tTeaching("speakHint")}
                      </span>
                    </div>
                    <button
                    ...
                    >
                      <Sparkles
                        className={`h-4 w-4 ${isDrawing ? "animate-spin" : ""}`}
                      />
