...
const SHAPE_KEYWORDS = ["圆形", "圆形在", "矩形", "方形", "正方形", "长方形", "方块", "直线", "线条", "线", "三角形", "五角星", "星星", "星", "圆"];
const ACTION_KEYWORDS = ["撤销", "重做", "清空", "清除", "重新开始", "保存", "存下来", "导出", "下载", "写上", "写下", "打字", "文字"];

export default function TranscriptBar({ transcript, isRecording, isProcessing }: TranscriptBarProps) {
    ...
    >
      <div className="flex-1 text-sm font-medium leading-relaxed text-text-primary">
        {isRecording && !transcript && (
          <span className="text-text-secondary transcript-cursor italic">正在倾听...</span>
        )}
        {isRecording && transcript && (
          <span className="text-text-secondary transcript-cursor">{highlightedContent}</span>
        )}
        {!isRecording && isProcessing && (
          <span className="text-text-secondary italic">正在解析画图指令...</span>
        )}
